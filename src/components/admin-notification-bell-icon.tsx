...
import { AdminNotificationSheet } from "./admin-notification-sheet"; // Import AdminNotificationSheet

export function AdminNotificationBellIcon() {
  ...
  const fetchUnreadCount = React.useCallback(async () => {
    if (!user || !isAdmin) { // Only for admin users
      setUnreadCount(0);
  ...
  if (isSessionLoading || isLoadingCount || !user || !isAdmin) {
    return null; // Only render for logged-in admin users
  }
  ...
  return (
    <>
      ...
      >
        ...
        <span className="sr-only">Notifikasi Admin</span>
      </Button>
      <AdminNotificationSheet open={isSheetOpen} onOpenChange={setIsSheetOpen} />
    </>
  );
}