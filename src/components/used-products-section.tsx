...
export function UsedProductsSection() {
  ...
  useEffect(() => {
    async function fetchUsedProducts() {
      setIsLoading(true);
      const hpBekas = await getProductsByCategory('hp-bekas');
      const laptopBekas = await getProductsByCategory('laptop-bekas');
      const produkBekas = await getProductsByCategory('produk-bekas');
      
      // Gabungkan semua produk bekas
      const combinedProducts = [...hpBekas, ...laptopBekas, ...produkBekas];
      setUsedProducts(combinedProducts);
      setIsLoading(false);
    }
    fetchUsedProducts();
  }, []);
